...
jQuery( function( $ ) {
	...
	FilePond.setOptions( {
		server: {
			process: function( inputElement, file, metadata, load, error, progress, abort ) {
				...
				formData.append( fieldName, file, file.name );console.log(formData);
				...
				jQuery.ajax( {
					...
					beforeSend: function ( xhr ) {
						xhr.setRequestHeader( 'X-WP-Nonce', toastUiEditorData.nonce );
					},
					data: formData
				} ).success( function( response ) {
    ...
    document.getElementById( 'toastui-media-button' ).addEventListener( 'click', function() {
		filepondContainer = document.getElementById( 'filepond-container' );
